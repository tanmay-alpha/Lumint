...
def run_lumint_experiment(manifest: DatasetManifest, module_name: str, config: Optional[ExperimentRunConfig] = None) -> ExperimentResult:
    ...
    for record in manifest.records:
        # If we specify a configuration for a specific module, make sure we only evaluate matching record types,
        # or we run all and handle errors. Let's dispatch to run_record.
